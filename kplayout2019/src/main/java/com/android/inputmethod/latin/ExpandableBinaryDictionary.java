...
abstract public class ExpandableBinaryDictionary extends Dictionary {
    ...
    protected static final String DICT_FILE_EXTENSION = ".jet";
    ...
    protected void addUnigramLocked(final String word, final int frequency,
            final boolean isNotAWord, final boolean isPossiblyOffensive, final int timestamp) {
        if (!mBinaryDictionary.addUnigramEntry(word, frequency,false /* isBeginningOfSentence */, isNotAWord, isPossiblyOffensive, timestamp)) {
            Log.d("dumpDicNasoale", "Cannot add unigram entry. word: " + word);
        }
    }
    ...
    public void dumpAllWordsForDebug() {
        reloadDictionaryIfRequired();
        final String tag = "dumpDic";
        final String dictName = mDictName;

        asyncExecuteTaskWithLock(mLock.readLock(), new Runnable() {
            @Override
            public void run() {
                Log.d("dumpDic", "Dump dictionary: " + dictName + " for " + mLocale);
                final BinaryDictionary binaryDictionary = getBinaryDictionary();
                Integer mTotalWords = 0;
                if (binaryDictionary == null) {
                    return;
                }
                try {
                ...
                do {
                  ...
                  //  Log.d(tag, wordProperty.mWord+"/"+wordProperty.mProbabilityInfo.mCount+"/"+wordProperty.getProbability()+"/"+wordProperty.mProbabilityInfo.mLevel+"/"+wordProperty.mProbabilityInfo.mProbability);
                    token = result.mNextToken;
                } while (token != 0);
                Log.d("dumpDics", "Total Words From Dic : " + mTotalWords);
            }
        });
    }
