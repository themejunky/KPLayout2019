...
public class SettingsValues {
    ...
    public int mCurrentIndexTheme;
    public final boolean mPrediction;
    ...
    public boolean isAddSpaceAutomatically = false;
    ...
    public void reloadTJ(final SharedPreferences prefs) {
        isAddSpaceAutomatically = prefs.getBoolean(Settings.PREF_ADD_SPACE_AUTOMATICALLY, false);
        mCurrentIndexTheme  = prefs.getInt(Settings.PREF_CURRENT_THEME, 0);
    }
    ...
    public SettingsValues(final Context context, final SharedPreferences prefs, final Resources res,
        ...
        mCurrentIndexTheme = 0;
        // Store the input attributes
        mInputAttributes = inputAttributes;

        // Get the settings preferences
        mAutoCap = prefs.getBoolean(Settings.PREF_AUTO_CAP, true);
        mKeyPreviewPopupOn = prefs.getBoolean(Settings.PREF_POPUP_ON, true);

        mVibrateOn = Settings.readVibrationEnabled(prefs, res);
        mSoundOn = Settings.readKeypressSoundEnabled(prefs, res);

        mSlidingKeyInputPreviewEnabled = prefs.getBoolean(DebugSettings.PREF_SLIDING_KEY_INPUT_PREVIEW, true);
        mShowsVoiceInputKey = needsToShowVoiceInputKey(prefs, res)
                && mInputAttributes.mShouldShowVoiceInputKey
                && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN;
        mIncludesOtherImesInLanguageSwitchList = Settings.ENABLE_SHOW_LANGUAGE_SWITCH_KEY_SETTINGS ? prefs.getBoolean(Settings.PREF_INCLUDE_OTHER_IMES_IN_LANGUAGE_SWITCH_LIST, false)
                : true /* forcibly */;
        mShowsLanguageSwitchKey = Settings.ENABLE_SHOW_LANGUAGE_SWITCH_KEY_SETTINGS
                ? Settings.readShowsLanguageSwitchKey(prefs) : true /* forcibly */;
        mUseContactsDict = prefs.getBoolean(Settings.PREF_KEY_USE_CONTACTS_DICT, true);
        mUsePersonalizedDicts = prefs.getBoolean(Settings.PREF_KEY_USE_PERSONALIZED_DICTS, true);
        mUseDoubleSpacePeriod = prefs.getBoolean(Settings.PREF_KEY_USE_DOUBLE_SPACE_PERIOD, true) && inputAttributes.mIsGeneralTextInput;
        mBlockPotentiallyOffensive = Settings.readBlockPotentiallyOffensive(prefs, res);
        mAutoCorrectEnabled = Settings.readAutoCorrectEnabled(prefs, res);
        final String autoCorrectionThresholdRawValue = mAutoCorrectEnabled ? res.getString(R.string.auto_correction_threshold_mode_index_modest) : res.getString(R.string.auto_correction_threshold_mode_index_off);
        ...
        mAutoCorrectionThreshold = readAutoCorrectionThreshold(res,autoCorrectionThresholdRawValue);
        ...
        mPrediction = prefs.getBoolean( Settings.PREF_ENABLE_PREDICTION, true);
    ...
    public int getCurrentIndexTheme() {
        return mCurrentIndexTheme;
    }

    public boolean isAddSpaceAutomatically() {
        return isAddSpaceAutomatically;
    }



    public boolean isAutomaticallyCapitalizeLetters() {
        return mAutoCap;
    }

    public boolean isKeyPreviewPopupOn() {
        return mKeyPreviewPopupOn;
    }
    ...
    private static float readAutoCorrectionThreshold(final Resources res, final String currentAutoCorrectionSetting) {
        final String[] autoCorrectionThresholdValues = res.getStringArray(R.array.auto_correction_threshold_values);
        ...
        Log.d("ajfkjhjfdjdfjkdfsjk2213",""+autoCorrectionThreshold);
