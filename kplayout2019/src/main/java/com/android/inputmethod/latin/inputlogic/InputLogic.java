...
public final class  InputLogic {
    ...
    public SuggestedWords suggestedWordsThree;
    ...
    public InputLogic(final LatinIME latinIME,
        ...
        Log.d("dictionarygsdg", "Skipping learning due to slow InputConnection.ss");
    }
    ...
    public InputTransaction onCodeInput(final SettingsValues settingsValues,@Nonnull final Event event, final int keyboardShiftMode,final int currentKeyboardScriptId, final LatinIME.UIHandler handler) {
        //adauga sunet
        Log.d("adaugare_dot2","1 : "+event.getTextToCommit().toString());
        ...
        //adds space after comma or dot
        if ((processedEvent.getTextToCommit().equals(".") || processedEvent.getTextToCommit().equals(",")) && settingsValues.isAddSpaceAutomatically) {
         // mLatinIME.getCurrentInputConnection().endBatchEdit();
            mLatinIME.getCurrentInputConnection().commitText(" ",0);
            mLatinIME.mHandler.moveCursorFront();
        }
    ...
    private void handleSeparatorEvent(final Event event, final InputTransaction inputTransaction,
        ...
        if (tryPerformSpaceAfterCommaOrPoint(event, inputTransaction)) {
            inputTransaction.setRequiresUpdateSuggestions();
    ...
    private boolean tryPerformDoubleSpacePeriod(final Event event, final InputTransaction inputTransaction) {
        ...
        if (!inputTransaction.mSettingsValues.mUseDoubleSpacePeriod                || Constants.CODE_SPACE != event.mCodePoint
        ...
        if (canBeFollowedByDoubleSpacePeriod(firstCodePoint)) {
            cancelDoubleSpacePeriodCountdown();
            mConnection.deleteTextBeforeCursor(1);
            final String textToInsert = inputTransaction.mSettingsValues.mSpacingAndPunctuations.mSentenceSeparatorAndSpace;
    ...
    private boolean tryPerformSpaceAfterCommaOrPoint(final Event event, final InputTransaction inputTransaction) {

        Log.d("ceva_interesant",""+       event.getTextToCommit());
    ...
    private void performAdditionToUserHistoryDictionary(final SettingsValues settingsValues,
        ...
        Log.d("dictionarygsdg", "Skipping learning due to slow InputConnection.");
...
//ieri
        ...
        mDictionaryFacilitator.addToUserHistory(suggestion, wasAutoCapitalized,ngramContext, timeStampInSeconds, settingsValues.mBlockPotentiallyOffensive);
      //  mDictionaryFacilitator.addToUserHistory("smsplus", wasAutoCapitalized, ngramContext, timeStampInSeconds, settingsValues.mBlockPotentiallyOffensive);
       // mDictionaryFacilitator.addToUserHistory("iconpack", wasAutoCapitalized,ngramContext, timeStampInSeconds, settingsValues.mBlockPotentiallyOffensive);
       // mDictionaryFacilitator.addToUserHistory("wallpapers", wasAutoCapitalized, ngramContext, timeStampInSeconds, settingsValues.mBlockPotentiallyOffensive);
       // mDictionaryFacilitator.addToUserHistory("keyboardplus", wasAutoCapitalized,ngramContext, timeStampInSeconds, settingsValues.mBlockPotentiallyOffensive);
        Log.d("theme_junky","xxxx 1 : "+suggestion+"/"+wasAutoCapitalized+"/"+ngramContext+"/"+timeStampInSeconds+"/"+settingsValues.mBlockPotentiallyOffensive);
      //  Log.d("theme_junky"," xxx 2  : "+mDictionaryFacilitator.getLocale());
       // mDictionaryFacilitator.dumpDictionaryForDebug(Dictionary.TYPE_MAIN);
      // mDictionaryFacilitator.dumpDictionaryForDebug(Dictionary.TYPE_CONTACTS);
        //mDictionaryFacilitator.dumpDictionaryForDebug(Dictionary.TYPE_USER_HISTORY);
      //  mDictionaryFacilitator.dump(mLatinIME);
        Log.d("theme_junky","xxxx 3");
        mDictionaryFacilitator.onFinishInput(mLatinIME);
    }
    ...
    public void performUpdateSuggestionStripSync(final SettingsValues settingsValues,
        ...
        if (suggestedWords != null) {
            ...
            suggestedWordsThree = suggestedWords;
