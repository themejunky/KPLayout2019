...
final class SuggestionStripLayoutHelper {
    ...
    private static final CharacterStyle BOLD_ITALIC = new StyleSpan(Typeface.ITALIC);
    ...
    private CharSequence getStyledSuggestedWord(final SuggestedWords suggestedWords,
        ...
        if ((isAutoCorrection && (options & AUTO_CORRECT_BOLD) != 0)
            ...
            suggestedWords.getInfo(indexInSuggestedWords).setBolded(true);
        }
    ...
    public int layoutAndReturnStartIndexOfMoreSuggestions(
        ...
        if (suggestedWords.size()>3) {
            ...
            String user_type = suggestedWords.getInfo(0).getWord().toLowerCase();

            String candidate_1 = suggestedWords.getInfo(1).getWord().toLowerCase();
            String candidate_2 = suggestedWords.getInfo(2).getWord().toLowerCase();
            String candidate_3 = suggestedWords.getInfo(3).getWord().toLowerCase();

            int userLenght = user_type.length();


            if (userLenght<=2) {
                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,1);
                LatinIME.candidates_type = 1;
            }
            else if ((new JaroWinkler().similarity(user_type,candidate_1)>=0.81 && userLenght==3) ) {
                //user typing first candidate
                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,2);
                LatinIME.candidates_type = 2;
            }
            else if ((new JaroWinkler().similarity(user_type,candidate_1)>=0.83 && userLenght==4) ) {
                //user typing first candidate
                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,2);
                LatinIME.candidates_type = 2;
            }

            else if ((new JaroWinkler().similarity(user_type,candidate_1)>=0.84 && userLenght==5)) {
                //user typing first candidate
                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,2);
                LatinIME.candidates_type = 2;
            }
            else if ((new JaroWinkler().similarity(user_type,candidate_1)>=0.89 && userLenght==6) ) {
                //user typing first candidate
                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,2);
                LatinIME.candidates_type = 2;
            }
            else if ((new JaroWinkler().similarity(user_type,candidate_1)>=91 && userLenght==7)) {
                //user typing first candidate
                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,2);
                LatinIME.candidates_type = 2;
            }
            else if ((new JaroWinkler().similarity(user_type,candidate_1)>=92 && userLenght==8) ) {
                //user typing first candidate
                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,2);
                LatinIME.candidates_type = 2;
            }
            else if ((new JaroWinkler().similarity(user_type,candidate_1)>=0.93 && userLenght==9)) {
                //user typing first candidate
                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,2);
                LatinIME.candidates_type = 2;
            }
            else if ((new JaroWinkler().similarity(user_type,candidate_1)>=0.94 && userLenght==10) ) {
                //user typing first candidate
                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,2);
                LatinIME.candidates_type = 2;
            }
            else if ((new JaroWinkler().similarity(user_type,candidate_1)>=0.95 && userLenght==11) ) {
                //user typing first candidate
                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,2);
                LatinIME.candidates_type = 2;
            }
            else if ((new JaroWinkler().similarity(user_type,candidate_1)>=0.96 && userLenght==12)) {
                //user typing first candidate
                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,2);
                LatinIME.candidates_type = 2;
            }
//            else if (candidate_1.startsWith(user_type) && (user_type.length()>=candidate_1.length()-2 || user_type.length()<=candidate_1.length()+2 )) {
//                startIndexOfMoreSuggestions = setupWordViewsAndReturnStartIndexOfMoreSuggestions(suggestedWords, mSuggestionsCountInStrip,2);
//                LatinIME.candidates_type = 2;
//                Log.d("candidatii","12");
//            }
      ...
      ThemeResource mTheme = DefaultThemes.getmInstance().getCurrentTheme();
        //daca nici un cuvant nu e maching perfect

        final int wordCountToShow = suggestedWords.getWordCountToShow(Settings.getInstance().getCurrent().mShouldShowLxxSuggestionUi);

        final TextView centerWordView = mWordViews.get(mCenterPositionInStrip);
        centerWordView.setTextColor(mTheme.keyLabel.normal);
        final int stripWidth = stripView.getWidth();
        final int centerWidth = getSuggestionWidth(mCenterPositionInStrip, stripWidth);
        if (wordCountToShow == 0 || getTextScaleX(centerWordView.getText(), centerWidth,
        ...
        for (int positionInStrip = 0; positionInStrip < countInStrip; positionInStrip++) {
            ...
            wordView.setTextColor(mTheme.keyLabel.normal);
    ...
    private int setupWordViewsAndReturnStartIndexOfMoreSuggestions(final SuggestedWords suggestedWords, final int maxSuggestionInStrip,final int type) {
        // Clear all suggestions first
        Log.d("testare_words","dus: "+type);
        ...
        for (indexInSuggestedWords = 0; indexInSuggestedWords < suggestedWords.size() && count < maxSuggestionInStrip; indexInSuggestedWords++) {

            int positionInStrip = 2 ;
            switch (type) {
                case 0 : positionInStrip =  getPositionInSuggestionStrip(indexInSuggestedWords, suggestedWords); break;
                case 1 :

                    if (indexInSuggestedWords==0) { positionInStrip=1; }
                    else if (indexInSuggestedWords==1) { positionInStrip=0; }
                    else if (indexInSuggestedWords==2) { positionInStrip=2; }

                    break;
                case 2 : positionInStrip = indexInSuggestedWords; break;
            }

            //final int positionInStrip =  getPositionInSuggestionStrip(indexInSuggestedWords, suggestedWords);
           // final int positionInStrip = indexInSuggestedWords;
            Log.d("feature_first_candidate","google_board");
