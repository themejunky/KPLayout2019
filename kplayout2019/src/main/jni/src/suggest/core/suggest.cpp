...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO   , "Suggest",__VA_ARGS__)
...
void Suggest::getSuggestions(ProximityInfo *pInfo, void *traverseSession,
    ...
    PROF_INIT;
    PROF_TIMER_START(0);   
        float maxSpatialDistance = 1.;
        if (TRAVERSAL)
            maxSpatialDistance = TRAVERSAL->getMaxSpatialDistance();
      
    DicTraverseSession *tSession = static_cast<DicTraverseSession *>(traverseSession);
        float maxPointerCount = 1.;
        if (TRAVERSAL)
            maxPointerCount = TRAVERSAL->getMaxPointerCount();
    tSession->setupForGetSuggestions(pInfo, inputCodePoints, inputSize, inputXs, inputYs, times,
            pointerIds, maxSpatialDistance, maxPointerCount);
    ...
    // keep expanding search dicNodes until all have terminated.
        if (tSession && tSession->getDicTraverseCache()) {
...
void Suggest::initializeSearch(DicTraverseSession *traverseSession) const {    
    ...
    if (traverseSession->getInputSize() > MIN_CONTINUOUS_SUGGESTION_INPUT_SIZE
        ...
        int maxCacheSize = 310;
        int terminalCacheSize = 18;
        if (TRAVERSAL) {
            maxCacheSize = TRAVERSAL->getMaxCacheSize(traverseSession->getInputSize(),
                                                      traverseSession->getSuggestOptions()->weightForLocale());
            terminalCacheSize = TRAVERSAL->getTerminalCacheSize();
        }
        // Restart recognition at the root.
        traverseSession->resetCache(maxCacheSize, terminalCacheSize);
...
void Suggest::expandCurrentDicNodes(DicTraverseSession *traverseSession) const {
    const int inputSize = traverseSession->getInputSize();
        int dicNodeSize = 60;
        if (TRAVERSAL){
            dicNodeSize = TRAVERSAL->getDefaultExpandDicNodeSize();
        }
    DicNodeVector childDicNodes(dicNodeSize);
    DicNode correctionDicNode;
        //LOGI("Suggest::expandCurrentDicNodes");
    // TODO: Find more efficient caching
    bool shouldDepthLevelCache = false;
        if (TRAVERSAL){
            shouldDepthLevelCache = TRAVERSAL->shouldDepthLevelCache(traverseSession);
        }
    ...
    while (traverseSession->getDicTraverseCache()->activeSize() > 0) {
        ...
        bool canDoLookAheadCorrection = false;
                if (TRAVERSAL)
                    canDoLookAheadCorrection = TRAVERSAL->canDoLookAheadCorrection(traverseSession, &dicNode);
        ...
        const bool isCompletion = dicNode.isCompletion(inputSize);
         ...
         bool shouldNodeLevelCache = false;
                 if (TRAVERSAL)
                     shouldNodeLevelCache = TRAVERSAL->shouldNodeLevelCache(traverseSession, &dicNode);
        ...
        if (dicNode.isInDigraph()) {
          ...
          //  LOGI("Suggest::expandCurrentDicNodes 3");
